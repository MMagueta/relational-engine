...
let write_and_retrieve () =
  ...
  in
  let+ (stream, locations), Command.ComputedHash handle =
    Command.commit_and_perform stream locations command_write1
  in
  ...
  in
  let+ (stream, locations), Command.ComputedHash _handle =
    Command.commit_and_perform stream locations command_write2
  in
  let+ (stream, locations), Command.ComputedHash handle =
    Command.commit_and_perform stream locations command_write3
  in
  ...
  in
  let+ (stream, locations), Command.ComputedHash _handle =
    Command.commit_and_perform stream locations command_write4
  in
  ...
  in
  let+ (stream, locations), Command.ComputedHash handle =
    Command.commit_and_perform stream locations command_write5
  in
  ...
  in
  let+ (stream, locations), Command.ComputedHash _handle =
    Command.commit_and_perform stream locations command_write6
  in
  let command_read1 : Command.t =
    {
      kind = Command.READ;
      timestamp = 0.0;
      hash = "";
  ...
  in
  let+ _, Command.Read first_name =
    Command.commit_and_perform (List.tl stream) locations command_read1
  in
  let command_read2 : Command.t =
    {
      kind = Command.READ;
      timestamp = 0.0;
      hash = "";
  ...
  in
  let+ _, Command.Read last_name =
    Command.commit_and_perform (List.tl stream) locations command_read2
  in
    ...
    [
      {
        ...
        tuples = List.map Bytes.to_string first_name;
      };
      {
        attribute_name = "user/last-name";
        attribute_type = "string";
        tuples = List.map Bytes.to_string last_name;
      };
    ]
  in
...
let write_and_retrieve_test () =
  ...
  let+ _, Command.Read relation =
    Command.commit_and_perform stream locations command_read1
  in
  (* let relation_result: Protocol.relation list = *)
