...
let%test _ =
  ...
  let ((new_commit, _), _) = Result.get_ok @@ write (List.hd stream) locations ~filename:"trash" @@ Bytes.of_string "abc" in
...
let%test _ =
  ...
  let stream: history = [{state = _EMPTY_SHA_HASH_; files = StringMap.empty}] in
  let locations: locations = StringMap.empty in
  let ((new_commit, new_locations), _) = Result.get_ok @@ write (List.hd stream) locations ~filename:"trash" @@ Bytes.of_string "abc" in
  let result_test =
